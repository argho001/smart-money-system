"""
Smart Money System - Real Backtester
Uses actual market signals (funding, sentiment, OI, volume, momentum).
"""
...
from modules.backtest.real_signal_simulator import RealSignalSimulator

class RealBacktester:
    def __init__(self, initial_capital=10000, risk_per_trade=1.0, max_positions=1):
    ...
    async def run(self, symbol="ETHUSDT", interval="4h", days_back=90,
                  entry_threshold=20, exit_threshold=-10, 
                  stop_loss_pct=3.0, take_profit_pct=6.0):
        """Run backtest with real signals"""
        
        print("=" * 60)
        print(f"REAL BACKTEST: {symbol} {interval} ({days_back} days)")
        print("=" * 60)
        ...
        signals = await simulator.prepare_signals(candles, symbol, days_back)
        ...
        for i, signal in enumerate(signals):
            ...
            # Check for new entry (only if signals agree)
            if len(self.positions) < self.max_positions:
                # Require at least 3 signals to agree
                bullish_signals = sum(1 for v in components.values() if v > 10)
                bearish_signals = sum(1 for v in components.values() if v < -10)
                
                if score > entry_threshold and bullish_signals >= 3:
                    self._open_position("LONG", price, timestamp, score, stop_loss_pct, take_profit_pct)
                elif score < -entry_threshold and bearish_signals >= 3:
        ...
        self._print_results(results, symbol, interval, days_back)
    ...
    def _open_position(self, side, price, timestamp, score, sl_pct, tp_pct):
        ...
        print(f"  📈 OPEN {side} @ ${price:,.2f} (score: {score:+.1f})")
    ...
    def _print_results(self, results, symbol, interval, days_back):
        ...
        print(f"""
📊 {symbol} {interval} ({days_back} days)
...
""")
        ...
        if wr >= 55 and pf >= 2.0 and sharpe >= 1.5:
            grade = "A+ (Excellent)"
        elif wr >= 50 and pf >= 1.5 and sharpe >= 1.0:
            grade = "A (Very Good)"
        elif wr >= 45 and pf >= 1.2 and sharpe >= 0.5:
            grade = "B (Good)"
        elif pf >= 1.0:
            grade = "C (Average)"
        else:
        ...
        filename = f"data/backtests/real_backtest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def run_real_backtest():
    """Run a complete backtest with real signals"""
